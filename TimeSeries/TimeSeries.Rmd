...
data <- diff(BJsales)
plot(BJsales)
plot(data) # diff to make stationary
print(adf.test(data))
acf(data) # geometric decay
pacf(data) # 2 significants, so AR(2)
ar(x = data, aic = FALSE, order.max = 2) # fit AR(2)

# the model gives us 

$$
\hat y_t = 0.2493y_{t-1}+0.2005y_{t-2}
$$
...
y_{t} &=z_{t}-z_{t-1} \\
z_{t} &=y_{t}+z_{t-1} \\
\hat{z}_{t} &=0.2493\left(z_{t-1}-z_{t-2}\right)+0.2005\left(z_{t-2}-z_{t-3}\right)+z_{t-1} \\
&=1.2493 z_{t-1}-0.0488 z_{t-2}-0.2005 z_{t-3}
\end{aligned}
$$
...
-   Série estacionária: quando ela se desenvolve no tempo aleatoriamente ao redor de uma média constante, refletindo alguma forma de equilíbrio estável.

    -   Estacionariedade homogênea: quando a série pode ser estacionária, flutuando ao redor de um nível, por um certo tempo, depois mudar de nível e flutuar ao redor de um novo nível e assim por diante, ou então mudar de inclinação.

    -   Estacionariedade não-homogênea:


-   is to remove that correlation

    -   By design (time invariant factors)
    -   By filtering (slowly changing factors) (slow upward in smoking and slow downward in airpollution may be a confounder, but not when looking to day by day changes)

-   We do this by resolving the data into different timescales.
    (removing the long-term trends resolves the relation with some confounder that may change over time)

-   Long term trends (here we can have confounders)

-   Slowly Changing Patterns (Season) (here we can have confounders)

-   Rest

The population is its own control

What do you mean by Filtering -\> short term fluctuations, few confounders.
Filter by using Splines for Time.
The decision of how much DF we choose for the spline is how short the timescale we want to focus on.
GCV or AIC is not a good way to decide how many DF.
You need to decide.
I use 4-5df per year of study.
Why?
Simulation suggests 4/yr is enough

In day to day, for pollution for ex.
we need to worry about: weather, temperature, barometrics that may be correlated

Time(date) is not a real variable.
Time is a surrogate for other variables we did not measure.

One other problem: Exposure is not randomly assigned, ex: people that chooses to to Hormone Replacement Therapy might be different from others aspects, and might be a counfounder.

# Poisson

Poisson regression provides a means of expressing the effect of the intervention in terms of the rate ratio.
It allows the analysis of aggregate data to be congruous with the analytic methods used in **case-control** and **cohort studies**[@kuhn1994].

Poisson distribution to analyze injury incidence.
It is considered the distribution of choice for rare events and has been selected as the appropriate stochastic model for natural variability of vital statistics, such as mortality rates.
